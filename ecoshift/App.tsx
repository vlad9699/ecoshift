...
import { District, DistrictStatus, PlayerStats, CampaignCity } from './types';
import { generateCityLevel, getMissionBriefing, generatePollutedCities } from './services/geminiService';
...
import { OperatorAvatar, LevelUpModal, CityCompleteModal, AchievementToast, Loader, LanguageSwitcher } from './components/ui';
...
const App: React.FC = () => {
  ...
  const [campaignCities, setCampaignCities] = useState<CampaignCity[]>(() => {
    // Try to load saved cities from localStorage
    try {
      const saved = localStorage.getItem('ECOSHIFT_SAVE_DATA_V1');
      if (saved) {
        const parsed = JSON.parse(saved);
        if (parsed.campaignCities && Array.isArray(parsed.campaignCities) && parsed.campaignCities.length > 0) {
          return parsed.campaignCities;
        }
      }
    } catch (e) {
  ...
  // Load AI-generated cities on mount (only if not already saved)
  useEffect(() => {
    const loadCities = async () => {
      setLoadingProgress(10);
      // Check if we already have saved cities
      try {
        const saved = localStorage.getItem('ECOSHIFT_SAVE_DATA_V1');
        if (saved) {
          const parsed = JSON.parse(saved);
          if (parsed.campaignCities && Array.isArray(parsed.campaignCities) && parsed.campaignCities.length > 0) {
            ...
            return; // Use saved cities, don't regenerate
          }
        }
      } catch (e) {
        console.error("Failed to check saved cities", e);
      }

      // Only generate new cities if we don't have saved ones
      setLoadingCities(true);
      setLoadingProgress(20);
      try {
        const aiCities = await generatePollutedCities(10, "environmental pollution, air quality, water contamination, and climate change", language);
        setLoadingProgress(50);
        if (aiCities.length > 0) {
          setCampaignCities(aiCities);
          // Save cities to localStorage
          try {
            const saved = localStorage.getItem('ECOSHIFT_SAVE_DATA_V1');
            const parsed = saved ? JSON.parse(saved) : {};
            parsed.campaignCities = aiCities;
            localStorage.setItem('ECOSHIFT_SAVE_DATA_V1', JSON.stringify(parsed));
          } catch (e) {
        ...
        console.warn("Failed to load AI cities, using defaults:", error);
        setLoadingProgress(40);
      } finally {
        setLoadingCities(false);
      }
  ...
  // Get current city (with fallback)
  // Try to find city by saved name first, then fall back to index
  const currentCity = useMemo((): CampaignCity => {
    if (campaignCities.length === 0) return CAMPAIGN_CITIES[0];
    
    // Try to find saved city name from localStorage
    try {
      const saved = localStorage.getItem('ECOSHIFT_SAVE_DATA_V1');
      if (saved) {
        const parsed = JSON.parse(saved);
        if (parsed.currentCityName) {
          const foundCity = campaignCities.find(c => c.name === parsed.currentCityName);
          if (foundCity) return foundCity;
        }
      }
    } catch (e) {
      console.error("Failed to load saved city name", e);
    }
    
    // Fall back to index-based selection
    return campaignCities[stats.campaignStage % campaignCities.length];
  }, [campaignCities, stats.campaignStage]);

  const currentCityIndex = useMemo(() => 
    campaignCities.findIndex(c => c.name === currentCity.name),
    [campaignCities, currentCity.name]
  );

  const nextCity = useMemo(() => 
    campaignCities.length > 0
      ? campaignCities[(currentCityIndex >= 0 ? currentCityIndex + 1 : stats.campaignStage + 1) % campaignCities.length]
      : CAMPAIGN_CITIES[1] || CAMPAIGN_CITIES[0],
    [campaignCities, currentCityIndex, stats.campaignStage]
  );

  // Save system - save stats, districts, current city name, and campaign cities
  useSaveSystem(stats, districts, currentCity.name, campaignCities);

  // Save current city name on first load if not already saved
  useEffect(() => {
    if (campaignCities.length > 0 && currentCity.name) {
      try {
        const saved = localStorage.getItem('ECOSHIFT_SAVE_DATA_V1');
        const parsed = saved ? JSON.parse(saved) : {};
        if (!parsed.currentCityName) {
          parsed.currentCityName = currentCity.name;
          localStorage.setItem('ECOSHIFT_SAVE_DATA_V1', JSON.stringify(parsed));
        }
      } catch (e) {
        console.error("Failed to save initial city name", e);
      }
    }
  }, [campaignCities.length, currentCity.name]);

  // Initial Load Logic
  useEffect(() => {
    // Don't load city data until cities are loaded
    if (loadingCities || campaignCities.length === 0) {
      return;
    }
  ...
  }, [stats.campaignStage, currentCity.name, loadingCities, campaignCities.length, districts.length, language]);
  ...
  const loadCityData = async (city: typeof currentCity) => {
    ...
    const data = await generateCityLevel(city.name, city.difficulty, language);
    setDistricts(data);
    setLoadingCity(false);
    setLoadingProgress(90);
    // Small delay to show 100% before hiding loader
  ...
  const selectDistrict = async (district: District) => {
    ...
    const txt = await getMissionBriefing(district, language);
    setBriefing(txt);
  };
  ...
  const handleTutorialClose = () => {
    setShowTutorial(false);
    // Don't mark as seen, so it can be shown again if needed
  };

  const travelToNextCity = () => {
    const nextCityIndex = (currentCityIndex >= 0 ? currentCityIndex + 1 : stats.campaignStage + 1) % campaignCities.length;
    const nextCityToTravel = campaignCities[nextCityIndex] || nextCity;
    ...
    // Save the next city name immediately
    try {
      const saved = localStorage.getItem('ECOSHIFT_SAVE_DATA_V1');
      const parsed = saved ? JSON.parse(saved) : {};
      parsed.currentCityName = nextCityToTravel.name;
      localStorage.setItem('ECOSHIFT_SAVE_DATA_V1', JSON.stringify(parsed));
    } catch (e) {
      console.error("Failed to save next city name", e);
    }
  ...
  const handleCollectTrash = (reward: { xp: number; coins: number; tokens: number }) => {
    const nextStats = {
      ...stats,
      xp: stats.xp + reward.xp,
      ecoCoins: stats.ecoCoins + reward.coins,
      impactTokens: stats.impactTokens + reward.tokens,
    };
    setStats(nextStats);
    const updates = checkProgression(nextStats);
    if (Object.keys(updates).length > 0) {
          ...
          <header className="flex flex-col md:flex-row justify-between md:items-end pb-6 border-b border-white/5 mb-6 gap-4 shrink-0">
            <div className="flex items-center gap-4 md:gap-5">
                ...
                {showPlayerInfo && (
                  <div className="absolute top-full left-0 mt-3 w-64 bg-slate-900/95 backdrop-blur-xl border border-emerald-400 p-5 rounded-2xl shadow-[0_0_50px_rgba(16,185,129,0.3)] text-xs origin-top-left transition-all duration-200 z-50">
                    ...
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <div>
                        <div className="text-slate-400 text-[10px] uppercase font-bold tracking-wider">{t.player.currentRank}</div>
                    ...
                    <div className="space-y-1 mb-4">
                      <div className="flex justify-between text-[10px] font-bold text-slate-400 uppercase">
                        <span>{t.player.progression}</span>
                        <span className="text-emerald-400">{Math.floor(stats.xp)} / {stats.xpToNextLevel} {t.player.xp}</span>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-4">
                      <div className="bg-slate-800/50 p-2 rounded border border-white/5 text-center">
                        <div className="text-[10px] text-slate-500 uppercase">{t.player.missions}</div>
                        <div className="text-white font-bold">{stats.missionsCompleted}</div>
                      </div>
                      <div className="bg-slate-800/50 p-2 rounded border border-white/5 text-center">
                        <div className="text-[10px] text-slate-500 uppercase">{t.player.restored}</div>
                    ...
                    >
                      <Trash2 size={12} /> {t.player.resetData}
                    </button>
                  </div>
                )}
              </div>
              ...
              <div>
                <h1 className="font-Fredoka text-2xl md:text-4xl font-bold text-white tracking-tight mb-1">
                  ECO<span className="text-emerald-500">SHIFT</span> <span className="hidden md:inline text-slate-600 text-lg">{t.player.operator}</span>
                </h1>
            ...
            <div className="flex items-center gap-3 w-full md:w-auto">
              <nav className="flex-1 md:flex-none flex bg-slate-900/50 p-1 rounded-lg border border-white/10 items-center gap-1 overflow-x-auto custom-scrollbar">
                ...
                >
                  <ShoppingBag size={16} /> <span className="hidden md:inline">{t.nav.hangar}</span> <span className="bg-black/20 px-1.5 rounded text-xs text-amber-400 whitespace-nowrap">{Math.floor(stats.ecoCoins)} G</span>
                </button>
                ...
                >
                  <Trophy size={16} /> <span className="hidden md:inline">{t.nav.achievements}</span>
                </button>
              </nav>
              ...
              >
                ...
                >
                  ...
                  <div className="flex flex-col leading-none">
                    ...
                    <span className="text-[9px] uppercase text-blue-500 font-bold tracking-wider group-hover:text-blue-300 transition-colors">{t.tokens.tokens}</span>
                  </div>
                </div>
                ...
                {showTokenInfo && (
                  <div className="absolute top-full right-0 mt-3 w-72 bg-slate-900/95 backdrop-blur-xl border border-blue-400 p-5 rounded-2xl shadow-[0_0_50px_rgba(37,99,235,0.3)] text-xs origin-top-right transition-all duration-200 z-50">
                    ...
                    <h4 className="text-cyan-300 font-black text-base mb-2 flex items-center gap-2">
                      <Gem size={16} className="animate-pulse" /> {t.tokens.impactToken}
                    </h4>
                    <p className="text-slate-300 mb-4 leading-relaxed">
                      {t.tokens.useTokenDescription}
                    </p>
                    <ul className="list-disc pl-4 space-y-1 text-slate-400 mb-4">
                      <li>{t.tokens.donateToCauses}</li>
                      <li>{t.tokens.inGameCurrency}</li>
                      <li>{t.tokens.rewardsFromPartners}</li>
                      <li>{t.tokens.plantRealTree}</li>
                    </ul>
          ...
          <div className="flex-1 min-h-0 overflow-hidden relative">
            ...
            {view === 'DASHBOARD' && (
              <Dashboard
                ...
                onGetBriefing={(district) => getMissionBriefing(district, language)}
                onUpdateCity={(updatedCity) => {
                  setCampaignCities(prev => 
